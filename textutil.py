...
class TextUtil(dict):
    ...
    def __init__(self, vocab_file=None, vocab=None):
        ...
        self.docs = []
        self.nlp = NLP().nlp
        self.vocab_lemmata = [[t.lemma_ for t in keyword] for keyword in self.nlp.pipe(map(str.lower, self.vocab))]
        self.dict = {" ".join(lem): kw for lem, kw in zip (self.vocab_lemmata, self.vocab)}
    ...
    def _strip(self,
               text: str,
               patterns: List[Pattern] = [RE_PUNCT, RE_NUMERIC, RE_SPACE]) -> str:
        '''
        @param text: string
        ...
        >>> Th sntnce has space numbr and punctuation
        '''
